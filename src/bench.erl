...
dist(Generators, N, NumGateways, P, Nodes, NumberOfGroups) ->
  {ok, Submaster_nodes}=grouping:get_submaster_nodes(Nodes,NumberOfGroups),
  {ok,master_group,Confirmed_Submaster_nodes}=s_group:new_s_group(master_group, Submaster_nodes), %% creates a group and add master and all submasters to it
  io:format("Number of all nodes: ~p\n", [length(Nodes)]),
  io:format("Number of nodes in the master group: ~p and confirmed ones: ~p \n", [length(Submaster_nodes), length(Confirmed_Submaster_nodes)]),
  case grouping:initiate(Nodes, NumberOfGroups) of %% recursively creates and returns all the submasters and group names
	{ok, Sub_masters} ->
		...
		sz(master:orbit(Generators(N), [0],NumGateways, P, 1, false, List_of_groups));
	Else ->
